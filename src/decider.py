...
class Decider:
    ...
    def init_chromosome(self, pattern):
        ...
        for k in range(self.states_num):
            rand = randint(0,len(random_list)-1)
    ...
    def file_matrix(self):
        ...
        for job in self.queue:
            mat = job.records_num
            if job.state == 'C1':
                matrix[job.file_name] = [(job.file_name + '_C1',mat/10**6), (job.file_name + '_C2',mat/20000),
                ...
                matrix[job.file_name] = [(job.file_name + '_C2', mat / 20000), (job.file_name + '_C3', mat / 200000)]
                states_index[job.file_name + '_C2'] = states_num
                states_index[job.file_name + '_C3'] = states_num + 1
                states_num += 2
            else:
                matrix[job.file_name] = [(job.file_name + '_C3', mat / 200000)]
    ...
    def mutation(self,chromosome):
        ...
        index_lst = sample(range(0,self.states_num), 3)
...
#     queue = [f1,f2,f3,f4]
#     decider = Decider(queue,4,0.5,0.25,0.25,1.0,0.25,100,2)
#     print(decider.exec())